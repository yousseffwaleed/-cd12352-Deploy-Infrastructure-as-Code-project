...
function Deploy-Stack([string] $StackName, [string] $TemplatePath, [string] $ParameterPath) {
    if (Test-StackExists $StackName) {
        ...
        $output = aws cloudformation update-stack --stack-name $StackName --template-body "file://$TemplatePath" --parameters "file://$ParameterPath" --capabilities CAPABILITY_IAM --region $Region 2>&1
        if ($LASTEXITCODE -ne 0) {
        ...
        aws cloudformation create-stack --stack-name $StackName --template-body "file://$TemplatePath" --parameters "file://$ParameterPath" --capabilities CAPABILITY_IAM --region $Region | Out-Null
...
aws cloudformation describe-stacks --stack-name $AppStackName --query 'Stacks[0].Outputs' --output table --region $Region
