...
function Remove-Stack([string] $StackName) {
    ...
    aws cloudformation delete-stack --stack-name $StackName --region $Region @AwsTlsArgs
    if ($LASTEXITCODE -ne 0) { throw "Could not delete $StackName." }
    aws cloudformation wait stack-delete-complete --stack-name $StackName --region $Region @AwsTlsArgs
