...
aws cloudformation validate-template --template-body "file://$RootDir/network.yml" --region $Region | Out-Null
if ($LASTEXITCODE -ne 0) { throw 'Network template validation failed.' }

aws cloudformation validate-template --template-body "file://$RootDir/udagram.yml" --region $Region | Out-Null
